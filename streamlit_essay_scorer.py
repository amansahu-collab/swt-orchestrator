...
def submit_essay_remark(essay_prompt: str, essay_text: str, scoring_response: dict,
                        expected_content: int, expected_structure: int, expected_linguistic: int,
                        remark: str, api_url: str, token: str):
    payload = {
        "essay_prompt": essay_prompt,
        "essay_text": essay_text,
        "scoring_response": scoring_response,
        "expected_content_score": expected_content,
        "expected_structure_score": expected_structure,
        "expected_linguistic_score": expected_linguistic,
        "remark": remark,
        "token": token
    }
    headers = {"Content-Type": "application/json", "X-API-TOKEN": token}
...
def main():
    ...
    with col1:
        essay_prompt = st.text_area("📋 Essay Prompt", height=120, placeholder="Enter the essay topic or question...")
    with col2:
        essay_text = st.text_area("✍️ Student Essay", height=120, placeholder="Paste the complete essay text here...")
    with col3:
    ...
    if st.session_state.get("essay_result"):
        ...
        st.markdown("If you think the scores should be different, enter the expected scores and a remark below.")

        fb_col1, fb_col2, fb_col3 = st.columns(3)
        with fb_col1:
            exp_content = st.number_input("Expected Content Score (0-6)", min_value=0, max_value=6,
                                          value=result.get("content_score", 0), step=1)
        with fb_col2:
            exp_structure = st.number_input("Expected Structure Score (0-6)", min_value=0, max_value=6,
                                            value=result.get("structure_score", 0), step=1)
        with fb_col3:
            exp_linguistic = st.number_input("Expected Linguistic Score (0-6)", min_value=0, max_value=6,
                                             value=result.get("linguistic_score", 0), step=1)

        remark = st.text_area("Remark", placeholder="Explain why you think the scores should be different...")

        if not st.session_state.get("remark_submitted"):
            if st.button("📤 Submit Feedback"):
                inserted_id = submit_essay_remark(
                    st.session_state["essay_prompt"],
                    st.session_state["essay_text"],
                    result,
                    exp_content, exp_structure, exp_linguistic,
                    remark, api_url, api_token
                )
                if inserted_id:
            ...
            st.success(f"Feedback submitted! MongoDB ID: {st.session_state.get('remark_id')}")
